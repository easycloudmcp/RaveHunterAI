...
Canonical persistence now serializes pricing, promoter, music, and media
metadata and records the event processing state. Existing databases are upgraded
additively when `create_tables` runs; the legacy table remains a documented
compatibility path and receives no Meta-specific columns.
